    ...
    <div className="page">
      ...
      <div className="pnl-card">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', marginBottom: '12px' }}>
          ...
          <div>
            <span className="pnl-label" style={{ fontSize: '0.75rem' }}>Current Value</span>
            <div style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--text)', marginTop: '2px' }}>
              ₹{Number(strategy.current_value || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <span className="pnl-label" style={{ fontSize: '0.75rem' }}>Total P&L</span>
            <div className={`pnl-value ${(strategy.total_pnl || 0) >= 0 ? 'positive' : 'negative'}`} style={{ fontSize: '1.1rem', fontWeight: 700, margin: '2px 0 0 0', justifyContent: 'flex-end' }}>
              {(strategy.total_pnl || 0) >= 0 ? '+' : ''}₹{Number(strategy.total_pnl || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </div>
          </div>
        </div>
      ...
      {strategy.webhook_enabled && (
        <div className="section" style={{ background: '#1a1a24', padding: '16px', borderRadius: '8px', border: '1px solid #334' }}>
          <div className="section-header" style={{ marginBottom: '12px' }}>
            <h3 style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Activity size={18} className="text-primary" /> TradingView Webhook
            </h3>
          </div>
          <div style={{ marginBottom: '12px' }}>
            <p className="text-muted" style={{ fontSize: '0.85rem', marginBottom: '4px' }}>Webhook URL</p>
            <div style={{ display: 'flex', gap: '8px' }}>
              <input 
                type="text" 
                readOnly 
                value={`https://subhoptionlab-7r1l.onrender.com/api/webhook/tradingview`}
              ...
              <button 
                ...
                onClick={() => {
                  navigator.clipboard.writeText('https://subhoptionlab-7r1l.onrender.com/api/webhook/tradingview')
          ...
          <div>
            <p className="text-muted" style={{ fontSize: '0.85rem', marginBottom: '4px' }}>JSON Payload Template (Paste into TV Alert Message)</p>
            <div style={{ position: 'relative' }}>
              <textarea
                readOnly
                className="input-large"
                style={{ width: '100%', height: '180px', fontFamily: 'monospace', fontSize: '0.8rem', whiteSpace: 'pre', background: '#000', color: '#0f0', border: '1px solid #333' }}
            ...
            <p className="field-hint" style={{ marginTop: '8px' }}>
              * Change "action" to "SELL" for short signals. Ensure plot_0 matches your SL line in TV.
            </p>
          </div>
          <div style={{ marginTop: '16px' }}>
            <button 
              className="btn-secondary" 
              onClick={() => navigate(`/strategies/${id}/webhook-logs`)}
              style={{ width: '100%', justifyContent: 'center' }}
            >
              <Activity size={16} style={{ marginRight: '8px' }}/> View Webhook Signal Logs
            </button>
          </div>
        </div>
      )}
